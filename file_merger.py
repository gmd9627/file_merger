...
# ✅ Function to copy sheets while skipping broken styles
    ...
    for sheet_name in source_wb.sheetnames:
        ...
        for row in source_ws.iter_rows():
            for cell in row:
                ...
                # ✅ Skip problematic styles instead of copying them
                try:
                    if hasattr(cell, "font"):
                        target_ws[cell.coordinate].font = cell.font
                    if hasattr(cell, "fill"):
                        target_ws[cell.coordinate].fill = cell.fill
                    if hasattr(cell, "border"):
                        target_ws[cell.coordinate].border = cell.border
                    if hasattr(cell, "alignment"):
                        target_ws[cell.coordinate].alignment = cell.alignment
                except Exception as e:
                    print(f"⚠️ Skipping corrupted style in {sheet_name} at {cell.coordinate}: {e}")
